...
| Limbu / Sirijonga | `namdhinggo`, `namdhinggosill`, `sirijonga`, `limbu` | Native reader for SIL's hash-pinned [`Limbu.map`](src/nepal_ttf2utf/maps/Limbu.map), including positional class rules and Unicode-order repair. Its supported forward `Byte_Unicode` subset fails closed on malformed or ambiguous rules. The source map explicitly leaves legacy `#` undefined. |
